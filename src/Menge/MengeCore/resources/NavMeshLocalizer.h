...
class MENGE_API NavMeshLocation {
 public:
   ...
   *previously a path, the path is deleted.
   */
...
class MENGE_API NavMeshLocalizer : public Resource {
 ...
 public:
   ...
   *search the full navigation mesh for the point.
   *	@returns	The index of the node associated with this location.
   *				If the location is not on a node, NavMeshLocation::NO_NODE is
   *returned.
   */
 ...
 protected:
   ...
   *selected. But giving an alternative value will change it to select a node nearest the current
   *elevation.
