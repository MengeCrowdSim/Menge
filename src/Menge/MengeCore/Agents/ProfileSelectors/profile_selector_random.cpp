...
bool ProfileSelectorRandom::cacheProfiles(HASH_MAP<std::string, AgentInitializer*>& profiles) {
  assert(_profiles.size() == 0 &&
         "Calling cacheProfiles on a random profile selector"
...
bool ProfileSelectorRandomFactory::setFromXML(ProfileSelector* pSel, TiXmlElement* node,
  ...
  assert(sel != 0x0 &&
         "Trying to set attributes of a random profile selector "
