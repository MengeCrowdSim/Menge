...
namespace MengeVis {
	namespace Runtime {
		...
		protected:
			 ...
			 */
			void drawORCALine( const Agent * agent,
							   const Menge::Math::Line & line, bool isAgent ) const;
		...
		template < class Agent >
		ORCATypeAgentContext< Agent >::ORCATypeAgentContext() : _showOrcaLines( false ),
			_visualizeORCA( false ), _visNbrID( 0 ) {
					...
					if ( noMods ) {
						if ( e.key.keysym.sym == SDLK_c ) {
							...
							if ( _visualizeORCA && this->_selected ) {
								const Agent * agt =
									dynamic_cast<const Agent *>( this->_selected->getAgent() );
								++_visNbrID;
		...
		void ORCATypeAgentContext< Agent >::drawORCALines( const Agent * agent ) const {
          ...
          glVertex3f( p1.x(), p1.y(), this->H );
				...
				for ( size_t i = FIRST_AGENT; i < LINE_COUNT; ++i ) {
					...
					this->writeAlignedText( ss.str(), nbr->_pos, SceneGraph::TextWriter::CENTERED,
											true );
				}
			}
		}
		...
		void ORCATypeAgentContext< Agent >::visORCA( const Agent * agt ) const {
			if ( _visualizeORCA && this->_selected ) {
				if ( agt->_nearAgents.size() > 0 ) {
					...
					const Agent * nbr =
						static_cast<const Agent *>( agent->_nearAgents[ _visNbrID ].agent );
          ...
          glTranslatef( center.x(), center.y(), this->H );
					...
					Menge::Math::Vector2 r1 = center + Rmin * Menge::Math::Vector2( cx - sy,
																					sx + cy );
					Menge::Math::Vector2 l1 = center + Rmin * Menge::Math::Vector2( cx + sy,
																					-sx + cy );
