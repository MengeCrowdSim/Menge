 ...
 *http://gamma.cs.unc.edu/PEDS/download/curtis12_FD.pdf It is a model of the underlying causes of
 *the so-called *fundamental diagram* -- the name for the density-dependent behavior observed in
 *pedestrians.  As the crowd gets denser, the crowd moves more slowly.  It uses two parameter:
 *stride factor and stride buffer.  The stride *factor* models physiological factors that correlate
 *available space to stride length (and therefore walking speed). A typical mean value is 1.57.  The
 *stride *buffer* models psychological factors which determine how the agent perceives the physical
 *space as a *comfortable* space. Typical values are in the range 0.5-0.9
 *
...
class FDMODIFIER_API FDModifierFactory : public Menge::BFSM::VelModFactory {
 ...
 protected:
   ...
   *relative to the behavior file location.  This is the folder containing that path.
